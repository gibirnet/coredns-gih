...
func (h *Hostsfile) parse(r io.Reader) *Map {
	...
	for scanner.Scan() {
		...
		for i := 1; i < len(f); i++ {
			...
			//log.Debugf("Host: %s , sha1: %s", name, string(f[i]))
...
func (h *Hostsfile) lookupStaticHost(m map[string][]net.IP, host string) []net.IP {
	...
	//log.Debugf("Host: %s , sha1: %s", host, GetHash(host))

	ips, ok := m[GetHash(strings.TrimRight(host, "."))+"."]
	ips2, ok2 := m[host]
	if (!ok && !ok2) {
		return nil
	}
